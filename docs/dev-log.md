...
- Tested whether the action branch's active/non-action (`other`) head should control exercise-mode exit. In a two-fold smoke (`_tsenyu_temp`, `_ziho_temp`), hard action-active gating reduced rest false reps only `38 -> 29` but worsened Count MAE `3.30 -> 4.10`. Event-level action-active evidence cut rest false reps to `10-12` and tail new-rest reps to `1`, but worsened Count MAE to `3.76-3.88`. Conclusion: use the other head as set-level confirmation/exit evidence, not as the primary active mask.
