"""
Action Classification 9-Fold LOSO (ExtraTrees ONLY - simple & fast)

ExtraTrees = Extremely Randomized Trees (simpler variant of RandomForest)
- Training: ~0.5s per fold
- Model size: ~200KB
- Inference: ~1ms per rep
- No ensemble, no hyperparameter search, just one model
"""
...
OUTPUT_DIR = Path("artifacts/action_classification_9fold_extratrees")
...
def main():
    ...
    print("="*70)
    print("Action Classification: 9-Fold LOSO (ExtraTrees ONLY)")
    print("="*70)
    ...
    for i, test_subj in enumerate(all_subjects):
        ...
        print(f"      reps: train={len(df_train)}, test={len(df_test)}")
        ...
        clf = ExtraTreesClassifier(n_estimators=200, max_depth=20, random_state=42, n_jobs=-1)
    ...
    print(f"\n{'='*70}")
    print("9-FOLD LOSO SUMMARY (ExtraTrees)")
    print(f"{'='*70}")
