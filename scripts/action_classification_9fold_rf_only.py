"""
Action Classification 9-Fold LOSO (RF ONLY - debug version)
"""
...
OUTPUT_DIR = Path("artifacts/action_classification_9fold_rf_only")
...
def main():
    ...
    print(f"[2/4] Loaded {len(sequences)} sequences from {len(all_subjects)} subjects")

    results = []
    for i, test_subj in enumerate(all_subjects):
        print(f"\n[3/4] Fold {i+1}/{len(all_subjects)}: test={test_subj}")
        ...
        print(f"      train reps={len(df_train)}, test reps={len(df_test)}, dim={df_train.shape[1]-1}")
    ...
    (OUTPUT_DIR / "rf_9fold_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    print(f"\nSaved to: {OUTPUT_DIR / 'rf_9fold_summary.json'}")
