"""
Action Classification 9-Fold LOSO (Simple Ensemble: RF + ExtraTrees + LogReg)

A lightweight, explainable ensemble using sklearn VotingClassifier.
Much simpler than AutoGluon's full AutoML pipeline.
"""
...
OUTPUT_DIR = Path("artifacts/action_classification_9fold_simple_ensemble")
...
def main():
    ...
    print("="*70)
    print("Action Classification: 9-Fold LOSO (Simple Ensemble)")
    print("Ensemble: RF + ExtraTrees + LogReg (soft voting)")
    print("="*70)
    ...
    for i, test_subj in enumerate(all_subjects):
        ...
        # Simple ensemble: 3 models with soft voting
        clf = VotingClassifier([
            ("rf", RandomForestClassifier(n_estimators=100, max_depth=15, random_state=42, n_jobs=-1)),
            ("xt", ExtraTreesClassifier(n_estimators=200, max_depth=20, random_state=42, n_jobs=-1)),
    ...
    print(f"\n{'='*70}")
    print("9-FOLD LOSO SUMMARY (Simple Ensemble)")
    print(f"{'='*70}")
