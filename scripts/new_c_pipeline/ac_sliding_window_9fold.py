"""
9-Fold LOSO: Global vs Action-Conditioned CNN
Data: Sliding window (back to original method that gave RepF1=0.848)
Architecture: Shared encoder + 8 independent 2-class heads
"""
...
class SlidingWindowDataset(Dataset):
    """Original method: sliding window with stride=150."""
    def __init__(self, segments, labels, actions=None, slice_len=300):
        self.samples = []
        self.has_action = actions is not None
        for i, (seq, lab) in enumerate(zip(segments, labels)):
            n = len(seq)
            if n <= slice_len:
                ...
                item = [torch.from_numpy(seq_pad[:slice_len]).float().transpose(0, 1),
                        torch.from_numpy(lab_pad[:slice_len]).long(),
                        torch.from_numpy(mask[:slice_len]).float()]
                if self.has_action:
                    item.append(torch.tensor(actions[i], dtype=torch.long))
                self.samples.append(tuple(item))
            else:
                stride = 200  # Faster than 150, still good overlap
                starts = list(range(0, n - slice_len + 1, stride))
                if not starts or starts[-1] + slice_len < n:
                    starts.append(n - slice_len)
                for start in starts:
                    item = [torch.from_numpy(seq[start:start+slice_len]).float().transpose(0, 1),
                            torch.from_numpy(lab[start:start+slice_len]).long(),
                            torch.from_numpy(np.ones(slice_len, dtype=np.float32))]
                    if self.has_action:
                        item.append(torch.tensor(actions[i], dtype=torch.long))
                    self.samples.append(tuple(item))
...
def train_fast(model, loader, val_loader, device, max_epochs=20, has_action=False):
    ...
    with torch.no_grad():
        for seg_start, seg_end in active_segments:
            ...
            if seg_len <= 300:
                ...
                stride = 200; starts = list(range(0, seg_len - 300 + 1, stride))
...
def main():
    ...
    for fold_idx, test_subject in enumerate(subjects):
        ...
        # Shared active detector
        active_models, active_scalers = train_active_detector(train_streams, cfg)
        
        # Extract data
        segs, labs, acts = extract_segments_with_action(train_streams, cfg.imu_columns)
        mean, std, norm_segs = normalize(segs)
        
        n_total = len(norm_segs); n_val = max(1, int(n_total * 0.15))
        indices = np.random.RandomState(42).permutation(n_total)
        train_idx, val_idx = indices[:-n_val], indices[-n_val:]
        
        # Global CNN
        print("  Training Global CNN...", end=" ")
        g_train = SlidingWindowDataset([norm_segs[i] for i in train_idx], [labs[i] for i in train_idx])
        g_val = SlidingWindowDataset([norm_segs[i] for i in val_idx], [labs[i] for i in val_idx])
        g_train_loader = DataLoader(g_train, batch_size=32, shuffle=True, drop_last=True)
        g_val_loader = DataLoader(g_val, batch_size=32, shuffle=False, drop_last=False)
        global_model = Global2ClassCNN(6, 64).to(device)
        global_model = train_fast(global_model, g_train_loader, g_val_loader, device, max_epochs=20, has_action=False)
        print("Done")
        
        # Action-Conditioned CNN
        print("  Training Action-Conditioned CNN...", end=" ")
        ac_train = SlidingWindowDataset([norm_segs[i] for i in train_idx], [labs[i] for i in train_idx], [acts[i] for i in train_idx])
        ac_val = SlidingWindowDataset([norm_segs[i] for i in val_idx], [labs[i] for i in val_idx], [acts[i] for i in val_idx])
        ac_train_loader = DataLoader(ac_train, batch_size=32, shuffle=True, drop_last=True)
        ac_val_loader = DataLoader(ac_val, batch_size=32, shuffle=False, drop_last=False)
        ac_model = ActionConditionedCNN(6, 64, 8).to(device)
        ac_model = train_fast(ac_model, ac_train_loader, ac_val_loader, device, max_epochs=20, has_action=True)
        print("Done")
        ...
        for stream_id, df in test_streams:
            ...
            g_probs = predict_global(global_model, df, active_segments, cfg.imu_columns, mean, std)
            g_results.append(evaluate_stream(stream_id, df, g_probs, gt_reps, gt_phases))
            
            a_idx = get_action_idx(stream_id)
            ac_probs = predict_ac(ac_model, df, active_segments, cfg.imu_columns, mean, std, a_idx)
    ...
    # Summary
    print(f"\n{'='*60}")
    print("9-FOLD SUMMARY (Sliding Window)")
    print(f"{'='*60}")
    ...
    # Verdict
    print(f"\n{'='*60}")
    if ac_summary['rep_f1'] > g_summary['rep_f1'] and ac_summary['exact_count_acc'] > g_summary['exact_count_acc']:
        print("VERDICT: ACTION-CONDITIONED ACCEPTED")
        print(f"  RepF1: {g_summary['rep_f1']:.4f} -> {ac_summary['rep_f1']:.4f}")
        print(f"  Exact: {g_summary['exact_count_acc']:.3f} -> {ac_summary['exact_count_acc']:.3f}")
    else:
        print("VERDICT: NO STRICT IMPROVEMENT")
        print(f"  Global baseline remains best")
    
    # Save
    out_dir = Path("artifacts/cnn_variant_comparison")
    out_dir.mkdir(parents=True, exist_ok=True)
    with open(out_dir / "ac_sliding_window_9fold.json", "w") as f:
    ...
    print(f"\n[OK] Saved to {out_dir / 'ac_sliding_window_9fold.json'}")
