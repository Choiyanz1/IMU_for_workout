...
With per-action post-processing sweep:
  - min_C_duration, min_E_duration, min_rep_duration, max_rep_duration
  - Global vs Per-Action decoder parameters
...
# ---------------------------------------------------------------------------
# Post-Processing Config (per-action)
# ---------------------------------------------------------------------------

@dataclass
class PostProcConfig:
    min_C_duration: int = 3       # min concentric samples
    min_E_duration: int = 3       # min eccentric samples
    min_rep_duration: int = 0     # min total rep samples (0=disable)
    max_rep_duration: int = 9999  # max total rep samples
    max_gap_samples: int = 3      # max gap between C and E when pairing


# Default global config
GLOBAL_PP = PostProcConfig()

# Per-action configs (initial guesses, will be tested)
PER_ACTION_PP = {
    "bench_press": PostProcConfig(min_C_duration=5, min_E_duration=5, min_rep_duration=20, max_rep_duration=400, max_gap_samples=3),
    "deadlift": PostProcConfig(min_C_duration=5, min_E_duration=5, min_rep_duration=20, max_rep_duration=500, max_gap_samples=3),
    "leg_press": PostProcConfig(min_C_duration=5, min_E_duration=5, min_rep_duration=20, max_rep_duration=400, max_gap_samples=3),
    "lat_pulldown": PostProcConfig(min_C_duration=5, min_E_duration=5, min_rep_duration=20, max_rep_duration=400, max_gap_samples=3),
    "shoulder_press": PostProcConfig(min_C_duration=5, min_E_duration=5, min_rep_duration=20, max_rep_duration=400, max_gap_samples=3),
    "bicep_curl": PostProcConfig(min_C_duration=5, min_E_duration=5, min_rep_duration=15, max_rep_duration=300, max_gap_samples=3),
    "leg_extension": PostProcConfig(min_C_duration=5, min_E_duration=5, min_rep_duration=15, max_rep_duration=300, max_gap_samples=3),
    "lunge": PostProcConfig(min_C_duration=5, min_E_duration=5, min_rep_duration=20, max_rep_duration=500, max_gap_samples=3),
}
...
# ---------------------------------------------------------------------------
# Rep / Phase parsing with post-processing config
# ---------------------------------------------------------------------------
...
def parse_reps_with_pp(hard_labels, pp: PostProcConfig, action: str = ""):
    """Parse reps with per-action post-processing."""
    pred_phase = np.array(["eccentric" if p == 0 else "concentric" for p in hard_labels])
    # Extract runs with per-phase minimum duration
    runs = labels_to_runs(
        pred_phase,
        positive_labels={"eccentric", "concentric"},
        min_length=1,  # Let us filter ourselves
    )
    if not runs:
        return []
    # Filter by min_C_duration / min_E_duration
    filtered = []
    for run in runs:
        if run.label == "concentric" and (run.end_idx - run.start_idx) < pp.min_C_duration:
            continue
        if run.label == "eccentric" and (run.end_idx - run.start_idx) < pp.min_E_duration:
            continue
        filtered.append(run)
    if not filtered:
        return []
    ...
    # Filter by total rep duration
    if pp.min_rep_duration > 0 or pp.max_rep_duration < 9999:
        reps = [r for r in reps
                if (r.end_idx - r.start_idx) >= pp.min_rep_duration
                and (r.end_idx - r.start_idx) <= pp.max_rep_duration]
    return reps
...
def extract_phase_segments(phase_arr, label: str):
    """Extract segments of a specific phase label as (start, end) tuples."""
    segments = []
...
def evaluate_phase_segments(pred_segments, gt_segments, iou_threshold=0.50):
    """Compute Precision, Recall, F1 for phase segment matching at IoU threshold."""
    tp = 0; matched_gt = set()
...
def evaluate_stream(stream_id, df, phase_probs, gt_reps, gt_phases, pp: PostProcConfig):
    pred_labels = np.argmax(phase_probs, axis=1)
    pred_reps = parse_reps_with_pp(pred_labels, pp)
    ...
    c_seg_metrics = evaluate_phase_segments(pred_c_segs, gt_c_segs, iou_threshold=0.50)
    e_seg_metrics = evaluate_phase_segments(pred_e_segs, gt_e_segs, iou_threshold=0.50)
    ...
    return {
        ...
        **phase_m,
        "concentric_seg_precision": c_seg_metrics["precision"],
        "concentric_seg_recall": c_seg_metrics["recall"],
        "concentric_seg_f1": c_seg_metrics["f1"],
        "eccentric_seg_precision": e_seg_metrics["precision"],
        "eccentric_seg_recall": e_seg_metrics["recall"],
        "eccentric_seg_f1": e_seg_metrics["f1"],
        **ce_metrics,
    }
...
def run_9fold(all_streams, subjects, cfg, output_dir):
    print("=" * 80)
    print("9-Fold LOSO: RF (Per-Action Window) vs CausalCNN + MA25+Viterbi")
    print("With per-action post-processing comparison")
    print("=" * 80)
    print(f"Subjects ({len(subjects)}): {subjects}")
    print(f"GPU: {torch.cuda.is_available()}")

    rf_fold_results = {"global_pp": [], "per_action_pp": []}
    cnn_fold_results = {"global_pp": [], "per_action_pp": []}
    ...
    for fold_idx, test_subject in enumerate(subjects):
        ...
        # Evaluate with both global and per-action post-processing
        for pp_mode in ["global", "per_action"]:
            rf_stream_results = []
            cnn_stream_results = []
            ...
            for stream_id, df in test_streams:
                ...
                # Choose post-processing config
                if pp_mode == "per_action" and action in PER_ACTION_PP:
                    pp = PER_ACTION_PP[action]
                else:
                    pp = GLOBAL_PP

                # RF
                rf_phase_probs = predict_rf_phase(rf_phase_models, rf_phase_scalers, stream_id, df, active_segments, cfg)
                rf_stream_results.append(evaluate_stream(stream_id, df, rf_phase_probs, gt_reps, gt_phases, pp))

                # CNN
                if cnn_model is not None:
                    cnn_phase_probs = predict_cnn_phase(cnn_model, df, active_segments, cfg.imu_columns, cnn_mean, cnn_std)
                    cnn_stream_results.append(evaluate_stream(stream_id, df, cnn_phase_probs, gt_reps, gt_phases, pp))

            # Aggregate
            rf_fold = aggregate_fold_results(rf_stream_results)
            rf_fold["fold"] = fold_idx + 1; rf_fold["test_subject"] = test_subject
            rf_fold_results[f"{pp_mode}_pp"].append(rf_fold)

            if cnn_model is not None:
                cnn_fold = aggregate_fold_results(cnn_stream_results)
                cnn_fold["fold"] = fold_idx + 1; cnn_fold["test_subject"] = test_subject
                cnn_fold_results[f"{pp_mode}_pp"].append(cnn_fold)

            print(f"  RF({pp_mode[:4]}):  RepF1={rf_fold['rep_f1']:.4f} Exact={rf_fold['exact_count_acc']:.3f} "
                  f"PhaseF1={rf_fold['phase_macro_f1']:.4f} TransMAE={rf_fold.get('transition_mae_ms', 0):.0f}ms "
                  f"CSegF1={rf_fold.get('concentric_seg_f1', 0):.3f} ESegF1={rf_fold.get('eccentric_seg_f1', 0):.3f} "
                  f"Over/Under={rf_fold['over_count']}/{rf_fold['under_count']}")
            if cnn_model is not None:
                print(f"  CNN({pp_mode[:4]}): RepF1={cnn_fold['rep_f1']:.4f} Exact={cnn_fold['exact_count_acc']:.3f} "
                      f"PhaseF1={cnn_fold['phase_macro_f1']:.4f} TransMAE={cnn_fold.get('transition_mae_ms', 0):.0f}ms "
                      f"CSegF1={cnn_fold.get('concentric_seg_f1', 0):.3f} ESegF1={cnn_fold.get('eccentric_seg_f1', 0):.3f} "
    ...
    for name, fold_results in [
        ("RF_GlobalPP", rf_fold_results["global_pp"]),
        ("RF_PerActionPP", rf_fold_results["per_action_pp"]),
        ("CNN_GlobalPP", cnn_fold_results["global_pp"]),
        ("CNN_PerActionPP", cnn_fold_results["per_action_pp"]),
    ]:
    ...
    # Head-to-head comparison (CNN Global vs RF Global)
    rf_g = rf_fold_results["global_pp"]
    cnn_g = cnn_fold_results["global_pp"]
    if rf_g and cnn_g:
        print(f"\n{'=' * 80}")
        print("CNN (Global PP) vs RF (Global PP) COMPARISON")
        print(f"{'=' * 80}")
        ...
        comparisons = {
            ...
            "concentric_seg_f1": "higher is better",
            "eccentric_seg_f1": "higher is better",
        }

        for metric, direction in comparisons.items():
            cnn_wins = 0
            for cnn_f, rf_f in zip(cnn_g, rf_g):
                ...
                if direction == "higher is better":
                    ...
                    if cnn_val < rf_val: cnn_wins += 1
            print(f"  {metric}: CNN wins in {cnn_wins}/{len(cnn_g)} folds")

    # Per-action PP improvement check
    cnn_g = cnn_fold_results["global_pp"]
    cnn_p = cnn_fold_results["per_action_pp"]
    if cnn_g and cnn_p:
        print(f"\n{'=' * 80}")
        print("CNN Per-Action PP vs Global PP")
        print(f"{'=' * 80}")
        for metric in ["rep_f1", "exact_count_acc", "concentric_seg_f1", "eccentric_seg_f1"]:
            wins = sum(1 for g, p in zip(cnn_g, cnn_p) if p.get(metric, 0) > g.get(metric, 0))
            print(f"  {metric}: Per-Action wins in {wins}/{len(cnn_g)} folds")

    # Save
    output_dir.mkdir(parents=True, exist_ok=True)
    out_path = output_dir / "comparison_9fold_per_action_pp.json"
    with open(out_path, "w") as f:
        json.dump({
            "summary": summary,
            "rf_global_pp": rf_fold_results["global_pp"],
            "rf_per_action_pp": rf_fold_results["per_action_pp"],
            "cnn_global_pp": cnn_fold_results["global_pp"],
            "cnn_per_action_pp": cnn_fold_results["per_action_pp"],
            "per_action_configs": {k: vars(v) for k, v in PER_ACTION_PP.items()},
