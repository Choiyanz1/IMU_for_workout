"""
9-Fold LOSO (Global 2-class CNN): EXCLUDE light-weight partial sessions

Excluded sessions (light weight, only 4 actions):
  - yanz/1000
  - thomas/thomas
  - thomas/thomas_2
  - kevin/kevin
...
Reports per-action:
  - Rep IoU-F1@0.50, Count MAE, Exact Count, Over/Under
  - Phase Macro F1, Transition MAE
...
def main():
    ...
    print(f"Total streams before exclusion: {len(all_streams)}")
    print(f"Subjects: {subjects}")
    print(f"Actions: {actions}")

    # Exclude specific sessions
    filtered_streams = [(sid, df) for sid, df in all_streams if not should_exclude(sid)]
    excluded_count = len(all_streams) - len(filtered_streams)
    print(f"Excluded streams: {excluded_count}")
    print(f"Remaining streams: {len(filtered_streams)}")
    print(f"GPU: {torch.cuda.is_available()}")

    cfg = PhaseCompareConfig()
    all_stream_results = []

    # 9-fold LOSO on filtered data
    ...
    print(f"\n{'='*80}")
    print("OVERALL (excluded light-weight partial sessions)")
    print(f"{'='*80}")
    print(f"Streams: {n}")
    ...
    with open(out_dir / "excluded_light_weight_per_action.json", "w") as f:
        json.dump({
            "overall": {"rep_f1": f1, "count_mae": float(np.mean(count_errors)), "n_streams": n},
            "per_action": by_action,
        }, f, indent=2, default=str)
    print(f"\n[OK] Saved to {out_dir / 'excluded_light_weight_per_action.json'}")
