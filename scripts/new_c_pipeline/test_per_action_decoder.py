"""
Quick test (kevin fold): Per-Action Decoder Parameters

Default: MA25 + Viterbi penalty=0.3 for all actions
Aggressive: Stronger smoothing + higher penalty for over-count prone actions

Per-Action Decoder Config:
  - db_bench_press:     MA40, penalty=0.5
  - db_rdl:             MA50, penalty=0.7 (worst over-counter)
  - db_shoulder_press:  MA40, penalty=0.5
  - db_weighted_crunch: MA40, penalty=0.5
  - Others:             MA25, penalty=0.3 (default)
"""
...
DEFAULT_DECODER = {"ma_window": 25, "viterbi_penalty": 0.3}

PER_ACTION_DECODER = {
    "db_bench_press": {"ma_window": 40, "viterbi_penalty": 0.5},
    "db_rdl": {"ma_window": 50, "viterbi_penalty": 0.7},
    "db_shoulder_press": {"ma_window": 40, "viterbi_penalty": 0.5},
    "db_weighted_crunch": {"ma_window": 40, "viterbi_penalty": 0.5},
    # Others use default
}
...
# ---------------------------------------------------------------------------
# Inference with per-action decoder
# ---------------------------------------------------------------------------
...
def predict_cnn_phase_with_decoder(model, df, active_segments, imu_columns, mean, std, action, decoder_config):
    """Predict with per-action decoder parameters."""
    cfg = decoder_config.get(action, DEFAULT_DECODER)
    ma_window = cfg["ma_window"]
    penalty = cfg["viterbi_penalty"]
    ...
    phase_probs = smooth_ma(phase_probs, ma_window)
    phase_probs = viterbi_decode(phase_probs, penalty)
    return phase_probs
...
def main():
    ...
    decoder_modes = [
        ("DEFAULT (MA25, p=0.3)", {}),
        ("PER-ACTION (Aggressive for bench/rdl/shoulder/crunch)", PER_ACTION_DECODER),
    ]

    all_results = {}
    for mode_name, decoder_cfg in decoder_modes:
        results = []
        for stream_id, df in test_streams:
            ...
            phase_probs = predict_cnn_phase_with_decoder(model, df, active_segments, cfg.imu_columns, mean, std, action, decoder_cfg)
            results.append(evaluate_stream(stream_id, df, phase_probs, gt_reps, gt_phases))

        agg = aggregate(results)
        by_action = aggregate_by_action(results)
        all_results[mode_name] = {"overall": agg, "per_action": by_action, "streams": results}
    ...
    # Compare worst over-count streams
    print(f"\n{'='*70}")
    print("WORST OVER-COUNT STREAMS COMPARISON")
    print(f"{'='*70}")
    default_results = all_results["DEFAULT (MA25, p=0.3)"]["streams"]
    per_action_results = all_results["PER-ACTION (Aggressive for bench/rdl/shoulder/crunch)"]["streams"]

    default_over = sorted([r for r in default_results if r["pred_count"] > r["gt_count"]],
                          key=lambda x: x["pred_count"] - x["gt_count"], reverse=True)[:10]

    print(f"\n{'Stream':<55s} {'Action':<20s} {'GT':>4s} {'Default':>8s} {'PerAction':>10s} {'Improvement':>12s}")
    print("-" * 115)
    for d in default_over:
        # Find matching per-action result
        pa = next((r for r in per_action_results if r["stream_id"] == d["stream_id"]), None)
        if pa:
            improvement = (d["pred_count"] - d["gt_count"]) - (pa["pred_count"] - pa["gt_count"])
            print(f"{d['stream_id']:<55s} {d['action']:<20s} {d['gt_count']:>4d} "
                  f"{d['pred_count']:>8d} {pa['pred_count']:>10d} {improvement:>11d}")

    # Save
    out_dir = Path("artifacts/cnn_variant_comparison")
    out_dir.mkdir(parents=True, exist_ok=True)
    with open(out_dir / "decoder_comparison_per_action.json", "w") as f:
        json.dump({
            "default_overall": all_results["DEFAULT (MA25, p=0.3)"]["overall"],
            "per_action_decoder_overall": all_results["PER-ACTION (Aggressive for bench/rdl/shoulder/crunch)"]["overall"],
            "default_per_action": all_results["DEFAULT (MA25, p=0.3)"]["per_action"],
            "per_action_decoder_per_action": all_results["PER-ACTION (Aggressive for bench/rdl/shoulder/crunch)"]["per_action"],
        }, f, indent=2, default=str)
    print(f"\n[OK] Saved to {out_dir / 'decoder_comparison_per_action.json'}")
