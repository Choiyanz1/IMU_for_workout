...
class BoundaryAwareTemporalUNet(nn.Module):
    def __init__(self, input_dim: int, num_classes: int, base_ch: int = 64, dropout: float = 0.1):
        super().__init__()
        c1 = base_ch
        c2 = base_ch * 2
        c3 = base_ch * 3

        self.enc1 = ConvBlock(input_dim, c1, dropout=dropout)
        self.down1 = nn.Conv1d(c1, c2, kernel_size=3, stride=2, padding=1)
        self.enc2 = ConvBlock(c2, c2, dropout=dropout)
        self.down2 = nn.Conv1d(c2, c3, kernel_size=3, stride=2, padding=1)
        self.enc3 = ConvBlock(c3, c3, dropout=dropout)

        self.bottleneck = nn.Sequential(
            nn.Conv1d(c3, c3, kernel_size=3, padding=2, dilation=2),
            nn.BatchNorm1d(c3),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Conv1d(c3, c3, kernel_size=3, padding=4, dilation=4),
            nn.BatchNorm1d(c3),
            nn.GELU(),
        )

        self.dec2 = ConvBlock(c3 + c2, c2, dropout=dropout)
        self.dec1 = ConvBlock(c2 + c1, c1, dropout=dropout)

        self.phase_head = nn.Conv1d(c1, num_classes, kernel_size=1)
        self.boundary_head = nn.Conv1d(c1, 3, kernel_size=1)
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        phase_logits = self.phase_head(d1).transpose(1, 2)
        boundary_logits = self.boundary_head(d1).transpose(1, 2)
...
def _phase_class_weights(train_streams, imu_columns: Sequence[str]) -> torch.Tensor:
    ...
    for _, df in train_streams:
        ...
        binc = np.bincount(idx, minlength=len(cb.MICRO_LABELS)).astype(np.float64)
        counts += binc
...
def train_model(model: nn.Module, train_streams, imu_columns: Sequence[str], slice_len: int, stride_len: int, epochs: int, lr: float, device: torch.device, boundary_loss_weight: float, boundary_focus: float, boundary_radius: int):
    ...
    pos_weight = torch.tensor([8.0, 6.0, 8.0], dtype=torch.float32, device=device)

    print(f"  [BoundaryUNet] Training on {len(ds)} slices, {epochs} epochs")
    for epoch in range(1, epochs + 1):
        ...
        if epoch % 10 == 0 or epoch == epochs:
            print(f"  [BoundaryUNet] epoch={epoch:03d}/{epochs:03d} loss={total_loss / max(1, count):.4f}")
...
def predict_model(model: nn.Module, df, imu_columns: Sequence[str], device: torch.device, smoothing_window: int = 9, boundary_threshold: float = 0.55):
    model.eval()
...
def main():
    parser = argparse.ArgumentParser(description="Train and evaluate a boundary-aware temporal U-Net for rep cutting.")
    parser.add_argument("--config", required=True)
    parser.add_argument("--output", default="artifacts/baseline_comparison/boundary_unet")
    ...
    model = BoundaryAwareTemporalUNet(input_dim=len(imu_columns), num_classes=len(cb.MICRO_LABELS), base_ch=64, dropout=0.1)
    n_params = sum(p.numel() for p in model.parameters())
    print(f"[INFO] model=BoundaryAwareTemporalUNet params={n_params:,}")
    ...
    results["model_name"] = "Boundary-Aware Temporal U-Net"
    ...
    md_lines = [
        "# Boundary-Aware Temporal U-Net Results\n",
